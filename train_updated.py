...
class TrainData(object):
	...
	def getBatch(self,batch_list):
		...
		for item in batch_list:
			...
			labels_array_norm = (label_array)/(255.0*1.1) #seems good
			#labels_array_norm = (label_array+255.0*0.05)/(255.0*1.1)
			#labels_array_norm = (label_array)/(255.0*1.2)
			#labels_array_norm = (label_array)/(255.0*1.15)
...
def main(args):
	...
	for epoch in range(epochs):
		for iters in range(int(math.ceil(1.0*data.num_data/batch_size))):
			...
			#tf.print("Current learning rate:", learning_rate, output_stream=sys.stdout)
			
		
		saver.save(sess, 'checkpoint/model_%d.ckpt' % epoch)
...
if __name__ == '__main__':
	...
	par.add_argument('--train_data_file',default='Data/trainData/trainDataLabel_all.txt',type=str,help='The training data file')
